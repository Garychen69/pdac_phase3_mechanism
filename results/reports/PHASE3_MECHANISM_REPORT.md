...
8b. **(New 2026-07-13) CAF subtype scoring is not independently reproducible.** Re-running `score_caf_subtypes` (`scripts/analyze_caf_emt_cell_of_origin.py`) on the full GSE202051 cohort twice, with identical code and identical fixed random seeds, produced qualitatively different dominant CAF subtypes each time (apCAF-dominant, then iCAF-dominant), and neither reproduced the myCAF-dominant percentages published in Section 5. The CAF subtype *proportions* reported in Section 5 should be treated as descriptive and not yet independently verified; this does not affect the EMT or lipid cell-of-origin findings, which are computed from separate, deterministic precomputed score files. Root cause not yet diagnosed — likely `sc.tl.score_genes`'s gene-binning step depending on data ordering/state not fully pinned by `random_state`. See Next Steps item 9.
...
9. **(New 2026-07-13) Diagnose and fix CAF-subtype-scoring non-determinism.** Two independent re-runs of `score_caf_subtypes` on the same full GSE202051 cohort with identical seeds gave qualitatively different dominant CAF subtypes (see Limitation 8b), neither matching the published Section 5 percentages. Needs root-cause investigation (likely in how `sc.tl.score_genes`'s control-gene binning interacts with `AnnData` view/copy state or a scanpy version-specific determinism gap) before the CAF subtype proportion numbers in Section 5 can be treated as a stable, citable result rather than descriptive/exploratory.
